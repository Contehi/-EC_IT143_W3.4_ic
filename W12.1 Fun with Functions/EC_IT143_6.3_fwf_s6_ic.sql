...
SELECT
    ...
    -- Ad-hoc first name extraction
    CASE
        WHEN CHARINDEX(',', ContactName) > 0 THEN LTRIM(RTRIM(SUBSTRING(ContactName, CHARINDEX(',', ContactName) + 1, 200)))
        WHEN CHARINDEX(' ', ContactName) > 0 THEN LEFT(ContactName, CHARINDEX(' ', ContactName) - 1)
        ELSE ContactName
    END AS First_AdHoc,

    -- Using scalar function
    dbo.ufn_GetFirstName(ContactName) AS First_UDF
